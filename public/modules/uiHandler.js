...
export function renderTable(dataToRender, elements) {
    ...
    sortedData.forEach(artist => {
        ...
        let artistNameHTML = `<div class="inline-flex items-center mt-1 px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/30 text-apple-blue dark:text-blue-400 text-[11px] font-medium border border-blue-100 dark:border-blue-800"><i data-lucide="mic-2" class="w-2.5 h-2.5 mr-1"></i>${editableSpan('Artiestennaam', artist.artistName, 'Artiestennaam')}</div>`;
        
        let instaHTML = `<div class="flex items-center text-gray-600 dark:text-gray-400 text-xs mt-1"><i data-lucide="instagram" class="w-3 h-3 mr-1.5 text-gray-400"></i> ${editableSpan('Instagram account', artist.instagram !== '-' ? artist.instagram : '', '@insta')}</div>`;
        
        let detailsHTML = '';
        detailsHTML += `<div class="flex items-center mb-1 text-xs text-gray-600 dark:text-gray-400"><i data-lucide="clock" class="w-3 h-3 mr-1.5 text-gray-400"></i> Speelduur: <strong class="ml-1 text-gray-900 dark:text-gray-200">${editableSpan('Speelduur', artist.setLength, '...')}</strong></div>`;
        ...
        let notesHTML = `<div class="text-xs text-gray-600 dark:text-gray-400 whitespace-pre-wrap leading-tight px-1.5 py-1 editable-text outline-none focus:bg-white dark:focus:bg-gray-700 focus:ring-1 focus:ring-apple-blue rounded border border-transparent hover:border-gray-200 dark:hover:border-gray-600 cursor-text min-h-[40px] empty:before:content-['Notities...'] empty:before:text-gray-400 transition-colors" contenteditable="true" data-field="Notities" data-row="${artist.rowIndex}" onblur="window.handleFieldBlur(event)">${artist.notes !== '-' ? artist.notes : ''}</div>`;
        
        let typeSelect = `<select data-field="Soort contact" data-row="${artist.rowIndex}" onchange="window.updateArtistField(event)" class="bg-transparent font-medium text-gray-900 dark:text-gray-100 text-sm outline-none cursor-pointer focus:ring-1 focus:ring-apple-blue rounded hover:bg-gray-100 dark:hover:bg-gray-700 py-0.5 px-1 -ml-1 transition-colors">
        ...
        tr.innerHTML = `
            <td class="px-2 py-1.5 text-center w-8"><input type="checkbox" class="rounded text-apple-blue focus:ring-apple-blue w-4 h-4 cursor-pointer mt-1" ${artist.mailingSelection ? 'checked' : ''} onchange="window.toggleMailingSelection(${artist.rowIndex}, this.checked)"></td>
            <td class="px-2 py-1.5 min-w-[140px]"><div class="font-medium text-gray-900 dark:text-gray-100 text-sm flex flex-wrap gap-1 mb-0.5">${editableSpan('Voornaam', artist.firstName, 'Voor')} ${editableSpan('Achternaam', artist.lastName, 'Achter')}</div>${artistNameHTML}</td>
            <td class="px-2 py-1.5 max-w-[150px]"><div class="flex flex-col gap-1"><div class="flex items-center text-gray-600 dark:text-gray-400 text-xs"><i data-lucide="mail" class="w-3 h-3 mr-1.5 text-gray-400 shrink-0"></i> <div class="truncate max-w-[120px]" title="${artist.email}">${editableSpan('E-mailadres', artist.email, 'E-mail')}</div></div><div class="flex items-center text-gray-600 dark:text-gray-400 text-xs"><i data-lucide="phone" class="w-3 h-3 mr-1.5 text-gray-400 shrink-0"></i> ${editableSpan('Telefoonnummer', artist.phone, 'Tel')}</div>${instaHTML}</div></td>
            <td class="px-2 py-1.5 w-[140px]"><div class="mb-1">${typeSelect}</div><div class="flex flex-col gap-1 text-xs text-gray-500 dark:text-gray-400"><label class="flex items-center cursor-pointer hover:text-gray-900 dark:hover:text-gray-200 transition-colors"><input type="checkbox" class="form-checkbox h-3 w-3 mr-1.5 rounded text-blue-500 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 focus:ring-blue-500" data-row="${artist.rowIndex}" data-field="Regio Den Haag" ${artist.regionDH ? 'checked' : ''} onchange="window.updateArtistField(event)"> Den Haag</label><label class="flex items-center cursor-pointer hover:text-gray-900 dark:hover:text-gray-200 transition-colors"><input type="checkbox" class="form-checkbox h-3 w-3 mr-1.5 rounded text-blue-500 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 focus:ring-blue-500" data-row="${artist.rowIndex}" data-field="Regio Rotterdam" ${artist.regionRdam ? 'checked' : ''} onchange="window.updateArtistField(event)"> Rotterdam</label></div></td>
            ...
            <td class="px-2 py-1.5 max-w-[180px] w-full">${notesHTML}</td>
            <td class="px-2 py-1.5 text-right w-10">
                <div class="flex flex-col items-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button data-index="${artist.rowIndex}" class="btn-delete text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 p-1.5 rounded-lg transition-colors focus:outline-none flex items-center justify-center" title="Verwijder Rij"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                </div>
            </td>`;
