...
export const createLineupItemHTML = (i, artist, num, starred = false, isFirstTimer = false, playedLast = false) => {
    const newBadge = artist.isNew ? '<span class="ml-1 text-xs font-semibold text-green-700 bg-green-100 dark:text-green-300 dark:bg-green-900/40 px-1.5 py-0.5 rounded">NIEUW</span>' : '';
    const firstTimeBadge = isFirstTimer ? '<span class="ml-1 text-xs font-semibold text-emerald-700 bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-900/40 px-1.5 py-0.5 rounded">1e KEER</span>' : '';
    const playedLastWarning = playedLast ? '<span class="ml-1 text-xs font-semibold text-red-700 bg-red-100 dark:text-red-300 dark:bg-red-900/40 px-1.5 py-0.5 rounded" title="Heeft vorige maand al gespeeld!">VORIGE MAAND</span>' : '';
    
    return `
    <div data-index="${i}" data-list="main" ondragstart="handleDragStart(event, ${i}, 'main')" ondragover="handleDragOver(event)" ondragenter="handleDragEnter(event)" ondragleave="handleDragLeave(event)" ondrop="handleDropOnMain(event, ${i})" ondragend="handleDragEnd(event); this.removeAttribute('draggable')" class="draggable-item flex items-center gap-3 p-3 ${playedLast ? 'bg-red-50/60 dark:bg-red-900/20 border-red-200 dark:border-red-800' : artist.isNew || isFirstTimer ? 'bg-green-50/60 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-gray-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-700'} rounded-lg border group transition-all hover:border-apple-blue/30">
...
export const createReserveItemHTML = (i, artist, starred = false, isFirstTimer = false, playedLast = false) => {
    const newBadge = artist.isNew ? '<span class="ml-1 text-xs font-semibold text-green-700 bg-green-100 dark:text-green-300 dark:bg-green-900/40 px-1.5 py-0.5 rounded">NIEUW</span>' : '';
    const firstTimeBadge = isFirstTimer ? '<span class="ml-1 text-xs font-semibold text-emerald-700 bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-900/40 px-1.5 py-0.5 rounded">1e KEER</span>' : '';
    const playedLastWarning = playedLast ? '<span class="ml-1 text-xs font-semibold text-red-700 bg-red-100 dark:text-red-300 dark:bg-red-900/40 px-1.5 py-0.5 rounded" title="Heeft vorige maand al gespeeld!">VORIGE MAAND</span>' : '';
    
    return `<div data-index="${i}" data-list="reserve" ondragstart="handleDragStart(event, ${i}, 'reserve')" ondragend="handleDragEnd(event); this.removeAttribute('draggable')" class="draggable-item flex items-center gap-3 p-3 ${playedLast ? 'bg-red-50/60 dark:bg-red-900/20 border-red-200 dark:border-red-800' : artist.isNew || isFirstTimer ? 'bg-green-50/60 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-white dark:bg-gray-800 border-orange-100 dark:border-orange-900/30'} rounded-lg border shadow-sm hover:border-orange-300 transition-colors"><div class="drag-handle flex items-center justify-center p-2 -ml-2 cursor-grab" onmousedown="this.closest('.draggable-item').setAttribute('draggable', 'true')" ontouchstart="this.closest('.draggable-item').setAttribute('draggable', 'true')" onmouseup="this.closest('.draggable-item').removeAttribute('draggable')" ontouchend="this.closest('.draggable-item').removeAttribute('draggable')" onmouseleave="this.closest('.draggable-item').removeAttribute('draggable')"><i data-lucide="grip-vertical" class="w-4 h-4 text-gray-400"></i></div><div class="flex-1 font-medium text-gray-900 dark:text-gray-100 flex items-center flex-wrap gap-1">${starred ? '⭐ ' : ''}${getDisplayName(artist)}${newBadge}${firstTimeBadge}${playedLastWarning}</div><button onclick="removeArtistFromReserve(${i})" class="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors"><i data-lucide="x" class="w-4 h-4"></i></button></div>`;
};

export const createCandidateItemHTML = (i, artist, isFirstTimer = false, playedLast = false) => {
    const newBadge = artist.isNew ? '<span class="ml-1 text-xs font-semibold text-green-700 bg-green-100 dark:text-green-300 dark:bg-green-900/40 px-1.5 py-0.5 rounded">NIEUW</span>' : '';
    const firstTimeBadge = isFirstTimer ? '<span class="ml-1 text-xs font-semibold text-emerald-700 bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-900/40 px-1.5 py-0.5 rounded">1e KEER</span>' : '';
    const playedLastWarning = playedLast ? '<span class="ml-1 text-xs font-semibold text-red-700 bg-red-100 dark:text-red-300 dark:bg-red-900/40 px-1.5 py-0.5 rounded" title="Heeft vorige maand al gespeeld!">VORIGE MAAND</span>' : '';
    
    return `<div data-index="${i}" data-list="candidates" ondragstart="handleDragStart(event, ${i}, 'candidates')" ondragend="handleDragEnd(event); this.removeAttribute('draggable')" class="draggable-item flex items-center gap-3 p-3 ${playedLast ? 'bg-red-50/60 dark:bg-red-900/20 border-red-200 dark:border-red-800' : artist.isNew || isFirstTimer ? 'bg-emerald-50/60 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800' : 'bg-white dark:bg-gray-800 border-blue-100 dark:border-blue-900/30'} rounded-lg border shadow-sm hover:border-blue-300 transition-colors"><div class="drag-handle flex items-center justify-center p-2 -ml-2 cursor-grab" onmousedown="this.closest('.draggable-item').setAttribute('draggable', 'true')" ontouchstart="this.closest('.draggable-item').setAttribute('draggable', 'true')" onmouseup="this.closest('.draggable-item').removeAttribute('draggable')" ontouchend="this.closest('.draggable-item').removeAttribute('draggable')" onmouseleave="this.closest('.draggable-item').removeAttribute('draggable')"><i data-lucide="grip-vertical" class="w-4 h-4 text-gray-400"></i></div><div class="flex-1 font-medium text-gray-900 dark:text-gray-100 flex items-center flex-wrap gap-1">${getDisplayName(artist)}${newBadge}${firstTimeBadge}${playedLastWarning}</div><button onclick="removeArtistFromCandidates(${i})" class="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors"><i data-lucide="x" class="w-4 h-4"></i></button></div>`;
};