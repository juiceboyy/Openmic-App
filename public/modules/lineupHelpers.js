...
function getRuleBadgesHTML(artist, isFirstTimer, playedLast, isOverridden, matchedPastNames) {
...
export const createLineupItemHTML = (i, artist, num, starred = false, isFirstTimer = false, playedLast = false, isOverridden = false, matchedPastNames = []) => {
    const badges = getRuleBadgesHTML(artist, isFirstTimer, playedLast, isOverridden, matchedPastNames);
    return `
...
export const createCandidateItemHTML = (i, artist, isFirstTimer = false, playedLast = false, isOverridden = false, matchedPastNames = []) => {
    const badges = getRuleBadgesHTML(artist, isFirstTimer, playedLast, isOverridden, matchedPastNames);
    return `<div data-index="${i}" data-list="candidates" ondragstart="handleDragStart(event, ${i}, 'candidates')" ondragend="handleDragEnd(event); this.removeAttribute('draggable')" class="draggable-item flex items-center gap-3 p-3 ${playedLast ? 'bg-red-50/60 dark:bg-red-900/20 border-red-200 dark:border-red-800' : artist.isNew || isFirstTimer ? 'bg-emerald-50/60 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800' : 'bg-white dark:bg-gray-800 border-blue-100 dark:border-blue-900/30'} rounded-lg border shadow-sm hover:border-blue-300 transition-colors"><div class="drag-handle flex items-center justify-center p-2 -ml-2 cursor-grab" onmousedown="this.closest('.draggable-item').setAttribute('draggable', 'true')" ontouchstart="this.closest('.draggable-item').setAttribute('draggable', 'true')" onmouseup="this.closest('.draggable-item').removeAttribute('draggable')" ontouchend="this.closest('.draggable-item').removeAttribute('draggable')" onmouseleave="this.closest('.draggable-item').removeAttribute('draggable')"><i data-lucide="grip-vertical" class="w-4 h-4 text-gray-400"></i></div><div class="flex-1 font-medium text-gray-900 dark:text-gray-100 flex items-center flex-wrap gap-1">${getDisplayName(artist)}${badges}</div><button onclick="removeArtistFromCandidates(${i})" class="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors"><i data-lucide="x" class="w-4 h-4"></i></button></div>`;
};