...
export const createLineupItemHTML = (i, artist, num, starred = false) => {
    ...
    return `
    <div data-index="${i}" data-list="main" ondragstart="handleDragStart(event, ${i}, 'main')" ondragover="handleDragOver(event)" ondragenter="handleDragEnter(event)" ondragleave="handleDragLeave(event)" ondrop="handleDropOnMain(event, ${i})" ondragend="handleDragEnd(event); this.removeAttribute('draggable')" class="draggable-item flex items-center gap-3 p-3 ${artist.isNew ? 'bg-green-50/60 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-gray-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-700'} rounded-lg border group transition-all hover:border-apple-blue/30">
...
export const createReserveItemHTML = (i, artist, starred = false) => {
    const newBadge = artist.isNew ? '<span class="ml-1 text-xs font-semibold text-green-700 bg-green-100 dark:text-green-300 dark:bg-green-900/40 px-1.5 py-0.5 rounded">NIEUW</span>' : '';
    return `<div data-index="${i}" data-list="reserve" ondragstart="handleDragStart(event, ${i}, 'reserve')" ondragend="handleDragEnd(event); this.removeAttribute('draggable')" class="draggable-item flex items-center gap-3 p-3 ${artist.isNew ? 'bg-green-50/60 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-white dark:bg-gray-800 border-orange-100 dark:border-orange-900/30'} rounded-lg border shadow-sm hover:border-orange-300 transition-colors"><div class="drag-handle flex items-center justify-center p-2 -ml-2 cursor-grab" onmousedown="this.closest('.draggable-item').setAttribute('draggable', 'true')" ontouchstart="this.closest('.draggable-item').setAttribute('draggable', 'true')" onmouseup="this.closest('.draggable-item').removeAttribute('draggable')" ontouchend="this.closest('.draggable-item').removeAttribute('draggable')" onmouseleave="this.closest('.draggable-item').removeAttribute('draggable')"><i data-lucide="grip-vertical" class="w-4 h-4 text-gray-400"></i></div><div class="flex-1 font-medium text-gray-900 dark:text-gray-100 flex items-center flex-wrap gap-1">${starred ? '⭐ ' : ''}${getDisplayName(artist)}${newBadge}</div><button onclick="removeArtistFromReserve(${i})" class="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors"><i data-lucide="x" class="w-4 h-4"></i></button></div>`;
};