...
app.post('/api/mailing', async (req, res) => {
  ...
  try {
    ...
    if (testMode) {
      // Overschrijf ontvanger in testmodus
      actualRecipients = [{ email: testEmail || 'haagseopenmic@gmail.com', name: 'Test Ontvanger' }];
    }

    // Bouw de Brevo payload op met messageVersions (elke ontvanger krijgt een eigen unieke mail)
    const messageVersions = actualRecipients.map(r => ({
      to: [{ email: r.email, name: r.name }]
    }));

    const brevoPayload = {
      sender: { name: 'Haagse Open Mic', email: 'haagseopenmic@gmail.com' },
    ...
    // Veilig uitlezen van de JSON reactie (Brevo kan soms een lege 201 sturen bij succes)
    let data;
